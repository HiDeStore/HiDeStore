...
void init_container_store() {
	...
	//NOTICE("init_container_store: %s", containerfile);
    ...
    NOTICE("Init container store successfully, container_count: %d", container_count);
}
...
void close_container_store() {
	...
	NOTICE("append phase stops successfully! the total number of contaienr is %ld", container_count);
...
void free_container_meta(void* cm) {
	g_hash_table_destroy( ((struct containerMeta*)cm)->map);
	free(cm);
}

void free_container(void* tc) {
...
int lookup_fingerprint_in_container_meta(void* cm,
		void *fp) {
	return g_hash_table_lookup( ((struct containerMeta*)cm)->map, (fingerprint *)fp) == NULL ? 0 : 1;
}

int lookup_fingerprint_in_container(void* c, void *fp) {
	return lookup_fingerprint_in_container_meta(&((struct container*)c)->meta, (fingerprint *)fp);
}
...
int container_check_id(void* c, void* id) {
	return container_meta_check_id(&((struct container*)c)->meta, (containerid*)id);
}
