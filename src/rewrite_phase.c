...
gint g_record_descmp_by_length(gconstpointer a,
		gconstpointer b, gpointer user_data) {
	return ((struct containerRecord*)b)->size - ((struct containerRecord*)a)->size;
}
...
gint g_record_cmp_by_id(gconstpointer a, gconstpointer b,
		gpointer user_data) {
	return ((struct containerRecord*)a)->cid - ((struct containerRecord*)b)->cid;
}
...
void start_rewrite_phase() {
    ...
    //NOTICE("rewrite_algorithm: no=0,cfl=1,cbr=2,cap=3: %d, [1]: %d", destor.rewrite_algorithm[0], destor.rewrite_algorithm[1]);
