...
extern void lpf_dedup(char *path);
 ...
 */
const char * const short_options = "sr::t::p::hac::u::";
...
void check_simulation_level(int last_level, int current_level) {
	if ((last_level <= SIMULATION_RESTORE && current_level >= SIMULATION_APPEND)
		...
		fprintf(stderr, "FATAL ERROR: Conflicting simualtion level! %d-->%d\n", last_level, current_level);
		exit(1);
	}
}
...
void destor_start() {
	...
	/* Init */
	destor.working_directory = sdsnew("/home/lpf/workspace/myworking/");
...
int main(int argc, char **argv) {
	...
	while ((opt = getopt_long(argc, argv, short_options, long_options, NULL))
		...
		case 'r':
			...
			break;
		case 'c':
			job = DESTOR_ACTIVE_RESTORE;
			revision = atoi(optarg);
			break;
		case 'u':
			job = DESTOR_ACTIVE_UPDATE_RECIPE;
			revision = atoi(optarg);
			break;
		case 'a':
			job = DESTOR_ACTIVE_BACKUP;
			break;
		case 's':
			destor_stat();
			break;
	...
	case DESTOR_ACTIVE_RESTORE:
		...
		lpf_restore(revision, path[0] == 0 ? 0 : path);

		sdsfree(path);
		break;
	case DESTOR_ACTIVE_UPDATE_RECIPE:
		if (revision <= 0) {
			fprintf(stderr, "A job id is required!\n");
			usage();
		}
		update_all_recipe(revision);
		break;
...
void free_chunk(void* ckk) {
	struct chunk* ck = (struct chunk*)ckk;
	if (ck->data) {
		free(ck->data);
		ck->data = NULL;
	}
...
void free_segment(gpointer ss) {
	struct segment* s = (struct segment*) ss;
	GSequenceIter *begin = g_sequence_get_begin_iter(s->chunks);
	GSequenceIter *end = g_sequence_get_end_iter(s->chunks);
	for(; begin != end; begin = g_sequence_get_begin_iter(s->chunks)){
		free_chunk(g_sequence_get(begin));
