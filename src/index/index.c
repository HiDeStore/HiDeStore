...
gboolean g_feature_equal(gconstpointer a, gconstpointer b){
    return !memcmp((char*)a, (char*)b, destor.index_key_size);
}
...
guint g_feature_hash(gconstpointer f){
    int i, hash = 0;
    char *feature = (char *)f;
    for(i=0; i<destor.index_key_size; i++){
        hash += feature[i] << (8*i);
...
void init_index() {
    ...
    NOTICE("Index exact=0 or near-exact=1: %d, physical=2 or logical=3: %d", destor.index_category[0], destor.index_category[1]);
    NOTICE("index_segment_selection_method, base=0,top=1,mix=2: %d, [1]: %d", destor.index_segment_selection_method[0], destor.index_segment_selection_method[1]);
    NOTICE("Index_specific: NO=0,DDFS=1,EX=2,SILO=3,SPARSE=4,SAMPLED=5,BLC=6: %d", destor.index_specific);
