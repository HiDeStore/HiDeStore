...
static gint g_segment_cmp_feature_num(gconstpointer aa,
		gconstpointer bb, gpointer user_data) {
	struct segment* a = (struct segment*)aa;
	struct segment* b = (struct segment*)bb;
	gint ret = g_hash_table_size(b->features) - g_hash_table_size(a->features);
	if (ret == 0) {
		ret = b->id > a->id ? 1 : -1;
		return ret;
...
static void features_trim(gpointer target,
		gpointer top) {
	GHashTableIter iter;
	gpointer key, value;
	g_hash_table_iter_init(&iter, ((struct segment *)top)->features);
	while (g_hash_table_iter_next(&iter, &key, &value)) {
		g_hash_table_remove( ((struct segment *)target)->features, key);
	}
}
