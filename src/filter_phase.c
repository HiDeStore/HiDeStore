...
void summary(int32_t id) {
    sds summaryfile = sdsdup(destor.working_directory);
    summaryfile = sdscat(summaryfile, "uni_dup_summary.txt");
    FILE *summary_fp;

    if ((summary_fp = fopen(summaryfile, "r+"))) {
        fseek(summary_fp, 0, SEEK_END);
    } else if (!(summary_fp = fopen(summaryfile, "w+"))) {
        perror(
                "Can not create uni_dup_summary.txt for read and write because");
        exit(1);
    }

    fwrite(&id, sizeof(id), 1, summary_fp);
    fwrite(&duplicate_num, sizeof(duplicate_num), 1, summary_fp);
    fwrite(&unique_num, sizeof(unique_num), 1, summary_fp);

    fclose(summary_fp);
}


void record_chunk_size(int32_t size) {
    sds chunkfile = sdsdup(destor.working_directory);
    chunkfile = sdscat(chunkfile, "chunk_size.txt");
    FILE *chunk_fp;

    if ((chunk_fp = fopen(chunkfile, "r+"))) {
        fseek(chunk_fp, 0, SEEK_END);
    } else if (!(chunk_fp = fopen(chunkfile, "w+"))) {
        perror(
                "Can not create chunk_size.txt for read and write because");
        exit(1);
    }

    fwrite(&size, sizeof(size), 1, chunk_fp);
    ...
    while (1) {
        ...
        for (; iter != end; iter = g_sequence_iter_next(iter)) {
        	...
        	if(r == NULL){
                ...
                VERBOSE("LPF: file name: %s", c->data);
...
void stop_filter_phase() {
    ...
    close_har();
	NOTICE("filter phase stops successfully! duplicate_num: %d, unique_num: %d", duplicate_num, unique_num);
